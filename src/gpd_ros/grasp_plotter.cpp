...
GraspPlotter::GraspPlotter(ros::NodeHandle& node,
...
visualization_msgs::MarkerArray GraspPlotter::convertToVisualGraspMsg(
    const std::vector<std::unique_ptr<gpd::candidate::Hand>>& hands,
    ...
    for (int i = 0; i < hands.size(); i++) {
        ...
        base = createHandBaseMarker(left_bottom, right_bottom,
                                    hands[i]->getFrame(), 0.02, hand_height_, i,
                                    frame_id);
        left_finger = createFingerMarker(left_center, hands[i]->getFrame(),
                                         finger_lwh, i * 3, frame_id);
        right_finger = createFingerMarker(right_center, hands[i]->getFrame(),
                                          finger_lwh, i * 3 + 1, frame_id);
        approach = createFingerMarker(approach_center, hands[i]->getFrame(),
...
visualization_msgs::Marker GraspPlotter::createFingerMarker(
    const Eigen::Vector3d& center, const Eigen::Matrix3d& frame,
...
visualization_msgs::Marker GraspPlotter::createHandBaseMarker(
