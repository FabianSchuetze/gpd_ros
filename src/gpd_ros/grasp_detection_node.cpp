...
const int GraspDetectionNode::CLOUD_SAMPLES =
    2;  ///< cloud with (x,y,z) samples
...
GraspDetectionNode::GraspDetectionNode(ros::NodeHandle& node)
    ...
    //  view_point_ << camera_position[0], camera_position[1],
    //  camera_position[2];
    ...
    if (!rviz_topic.empty()) {
        grasps_rviz_pub_ =
            node.advertise<visualization_msgs::MarkerArray>(rviz_topic, 1);
        use_rviz_ = true;
    } else {
        use_rviz_ = false;
    }
    ...
    if (cloud_type == POINT_CLOUD_2) {
        cloud_sub_ = node.subscribe(cloud_topic, 1,
                                    &GraspDetectionNode::cloud_callback, this);
    } else if (cloud_type == CLOUD_INDEXED) {
        cloud_sub_ = node.subscribe(
            cloud_topic, 1, &GraspDetectionNode::cloud_indexed_callback, this);
    } else if (cloud_type == CLOUD_SAMPLES) {
        cloud_sub_ = node.subscribe(
            cloud_topic, 1, &GraspDetectionNode::cloud_samples_callback, this);
    ...
    if (!samples_topic.empty()) {
        samples_sub_ = node.subscribe(
            samples_topic, 1, &GraspDetectionNode::samples_callback, this);
        has_samples_ = false;
    }

    // uses ROS topics to publish grasp candidates, antipodal grasps, and grasps
    // after clustering
    grasps_pub_ =
        node.advertise<gpd_ros::GraspConfigList>("clustered_grasps", 10);

    rviz_plotter_ = new GraspPlotter(
        node, grasp_detector_->getHandSearchParameters().hand_geometry_);
...
void GraspDetectionNode::run() {
    ...
    while (ros::ok()) {
        if (has_cloud_) {
            ROS_INFO("Cloud arrived, working on the grasp generation");
...
GraspDetectionNode::detectGraspPoses() {
    ...
    ROS_INFO_STREAM("Published " << selected_grasps_msg.grasps.size()
...
std::vector<int> GraspDetectionNode::getSamplesInBall(
    const PointCloudRGBA::Ptr& cloud, const pcl::PointXYZRGBA& centroid,
    float radius) {
...
void GraspDetectionNode::cloud_indexed_callback(
    const gpd_ros::CloudIndexed& msg) {
    if (!has_cloud_) {
        ...
        ROS_INFO_STREAM("Received cloud with "
                        << cloud_camera_->getCloudProcessed()->size()
                        << " points, and " << msg.indices.size() << " samples");
    }
}

void GraspDetectionNode::cloud_samples_callback(
    const gpd_ros::CloudSamples& msg) {
    if (!has_cloud_) {
        initCloudCamera(msg.cloud_sources);
        ...
        for (int i = 0; i < msg.samples.size(); i++) {
            samples.col(i) << msg.samples[i].x, msg.samples[i].y,
                msg.samples[i].z;
        }
        ...
        ROS_INFO_STREAM("Received cloud with "
                        << cloud_camera_->getCloudProcessed()->size()
                        << " points, and " << cloud_camera_->getSamples().cols()
                        << " samples");
    }
}
...
void GraspDetectionNode::samples_callback(const gpd_ros::SamplesMsg& msg) {
    if (!has_samples_) {
        ...
        ROS_INFO_STREAM("Received grasp samples message with "
                        << msg.samples.size() << " samples");
    }
}
...
void GraspDetectionNode::initCloudCamera(const gpd_ros::CloudSources& msg) {
    ...
    for (int i = 0; i < msg.view_points.size(); i++) {
        view_points.col(i) << msg.view_points[i].x, msg.view_points[i].y,
            msg.view_points[i].z;
    }
